                       ...
                       unifyfs_args_t* args)
{
    ...
    snprintf(cmd, sizeof(cmd),
             "jsrun --immediate -e -individual --stdio_stderr unifyfs-stage.err.%%h.%%p --stdio_stdout unifyfs-stage.out.%%h.%%p --nrs %s -r1 -c1 -a1",
             n_nodes);
                        ...
                        unifyfs_args_t* args)
{
    ...
    // full command: mpirun <mpirun args> <server args>
    snprintf(n_nodes, sizeof(n_nodes), "%zu", resource->n_nodes);
    snprintf(cmd, sizeof(cmd), "mpirun --np %s --map-by ppr:1:node", n_nodes);
                      ...
                      unifyfs_args_t* args)
{
    ...
    // full command: srun <srun args> <server args>
    snprintf(n_nodes, sizeof(n_nodes), "%zu", resource->n_nodes);
    snprintf(cmd, sizeof(cmd), "srun -N %s --ntasks-per-node 1", n_nodes);
